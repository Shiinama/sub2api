			...
			t.Run(endpoint+"/"+tc.name, func(t *testing.T) {
				...
				if endpoint == "messages" {
					req, wireBody, err = svc.buildUpstreamRequest(context.Background(), c, account,
						body, "test-token", "oauth", "claude-haiku-4-5", false, claudeUpstreamForwardingMode{mimicClaudeCode: tc.mimic})
				} else {
					req, wireBody, err = svc.buildCountTokensRequest(context.Background(), c, account,
						body, "test-token", "oauth", "claude-haiku-4-5", claudeUpstreamForwardingMode{mimicClaudeCode: tc.mimic})
				}
				require.NoError(t, err)
