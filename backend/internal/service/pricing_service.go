...
func (s *PricingService) parsePricingData(body []byte) (map[string]*LiteLLMModelPricing, error) {
	...
	for modelName, rawEntry := range rawData {
		...
		applyPriorityLongContextTier(rawEntry, pricing)
...
// applyPriorityLongContextTier preserves independent priority prices at the standard
// tier threshold. Explicit long_context fields, including zero, remain authoritative.
func applyPriorityLongContextTier(rawEntry json.RawMessage, pricing *LiteLLMModelPricing) {
	if pricing.LongContextInputTokenThreshold <= 0 {
		return
	}
	var fields map[string]float64
	// Entries also contain strings and booleans, so decode individual prices below.
	...
	suffix := fmt.Sprintf("_above_%dk_tokens_priority", pricing.LongContextInputTokenThreshold/1000)
	ratio := func(base float64, key string) float64 {
		if base <= 0 || fields[key] <= 0 {
			return 0
		}
		return fields[key] / base
	}
	input := ratio(pricing.InputCostPerTokenPriority, "input_cost_per_token"+suffix)
	if input <= 0 {
		input = ratio(pricing.CacheReadInputTokenCostPriority, "cache_read_input_token_cost"+suffix)
	}
	output := ratio(pricing.OutputCostPerTokenPriority, "output_cost_per_token"+suffix)
	if input <= 0 && output <= 0 {
		return
	}
	pricing.LongContextPriorityPricing = true
	if _, explicit := rawFields["long_context_input_cost_multiplier_priority"]; !explicit {
		pricing.LongContextInputCostMultiplierPriority = longContextMultiplierOrOne(input)
	}
	if _, explicit := rawFields["long_context_output_cost_multiplier_priority"]; !explicit {
		pricing.LongContextOutputCostMultiplierPriority = longContextMultiplierOrOne(output)
	}
}
