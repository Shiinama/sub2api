...
func TestCalculateCost_GPTLongContextAppliesToFlexAndPriorityWithStandardMultipliers(t *testing.T) {
	pricingSvc := &PricingService{
		pricingData: map[string]*LiteLLMModelPricing{
			"gpt-5.5": {
	...
	require.NoError(t, err)
	require.InDelta(t, float64(tokens.InputTokens)*5e-6*2.5*2.0, priorityCost.InputCost, 1e-10)
	require.InDelta(t, float64(tokens.CacheCreationTokens)*5e-6*2.5*2.0, priorityCost.CacheCreationCost, 1e-10)
	require.InDelta(t, float64(tokens.CacheReadTokens)*0.5e-6*2.5*2.0, priorityCost.CacheReadCost, 1e-10)
	require.InDelta(t, float64(tokens.OutputTokens)*30e-6*2.5*1.5, priorityCost.OutputCost, 1e-10)
}
...
func TestCalculateCost_GeminiUsesModelLevelWholeSessionPricing(t *testing.T) {
	pricingSvc := &PricingService{
		pricingData: map[string]*LiteLLMModelPricing{
			"gemini-2.5-pro": {
				InputCostPerToken:               1.25e-6,
	...
	cost, err := svc.CalculateCost("gemini-2.5-pro", tokens, 1.0)
	require.NoError(t, err)
...
func TestCalculateCost_MarkerRequiresActualCostIncrease(t *testing.T) {
	svc := newTestBillingService()
	tokens := UsageTokens{InputTokens: 300000}

	cost, err := svc.CalculateCost("claude-sonnet-4", tokens, 0)
