...
func (s *GatewayService) ForwardAsChatCompletions(
	...
	// 6. Apply Claude Code mimicry for OAuth accounts when enabled.
	// Chat Completions 协议进来的请求永远不是 Claude Code 客户端，所以对 OAuth 账号
	// 默认执行 /v1/messages 主路径上的伪装链路（system 重写 + normalize + metadata 注入）；
	...
	upstreamReq, _, err := s.buildUpstreamRequest(upstreamCtx, c, account, anthropicBody, token, tokenType, mappedModel, reqStream, forwardingMode)
	releaseUpstreamCtx()
	if err != nil {
